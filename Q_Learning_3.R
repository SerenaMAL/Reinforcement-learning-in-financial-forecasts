...
tick <- 10 # discretization step for the state generation
...
while( E <= episodes+refining.episodes ) {
  ...
  while( t <= length(price.trn) ) {
    ...
    # update Q table
    ret_ <- diff( price.trn[(t+dt-win+1):(t+dt)] )
    # s_ <- round(c( cash, mean(ret_), sd(ret_) )/tick)
    s_ <- c( round((cash-initial.cash)/tick), round(mean(ret_)/tick,1), round(sd(ret_)/tick,1) ) 
    
    jj <- which( Q$cash==s_[1] & Q$r.m==s_[2] & Q$r.s==s_[3] )
      
    if( t==length(price.trn[t]) || length(jj)==0 ) {
      Q.max <- 0
    } else {
      Q.max <- max(Q$q[jj])
    }
...
all.cash.tst <- matrix(NA,1,length(price.trn))
all.reward.tst <- matrix(NA,1,length(price.trn))
all.actions.tst <- matrix("",1,length(price.trn))
...
legend("topleft",legend=paste("final cash:",round(cash)))
